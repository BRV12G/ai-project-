...
# class to create cells 
class Cell: #for cells to remember its position itself that is the use of self
	def __init__(self,r,c):
		...
		self.sides = [False, False, False, False] #to draw sides on the dots
		self.winner = None
...
def reset_cells():
	pos = None #to store the positio of the mouse pointer
	ccell = None
	up = False
	right = False
	bottom = False
...
def reset_Score():
	fillcount=0 #counts how many boxes ahve been filled
	p1_score=0
	p2_score=0
	return fillcount,p1_score,p2_score

def reset_player():#implementing the players X and O 
	turn =0
	players = ['X', 'O']
	player = players[turn]
	next_turn = False
...
#for the running screen display
running = True
while running:
	win.fill(BLACK)
	...
	if ccell:
		index = ccell.index
		if not ccell.winner:
			pygame.draw.circle(win, RED, (cell.rect.centerx, cell.rect.centery),2)
	...
	p1img = font.render(f'Player 1(X) : {p1_score}', True, BLUE)
	p1rect = p1img.get_rect()
	p1rect.x, p1rect.y = 2*PADDING , 15

	p2img = font.render(f'Player 2(O): {p2_score}', True, BLUE)
	...
	# turning = font.render(f'{player}', True, WHITE)
	# win.blit(turning,(0,0))
